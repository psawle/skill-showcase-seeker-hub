...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-16 sm:py-20 px-4 sm:px-6 bg-slate-800/30">
      <div className="container mx-auto max-w-6xl">
          ...
          {skillCategories.map((category, index) => (
            <Card key={category.title} className="bg-slate-800/50 border-slate-700 hover:bg-slate-800/70 transition-all duration-300 hover:transform hover:scale-105">
              <CardContent className="p-4 sm:p-6">
                <h3 className={`text-lg sm:text-xl font-semibold mb-4 bg-gradient-to-r ${category.color} bg-clip-text text-transparent`}>
                  {category.title}
                </h3>
        ...
        <div className="mt-12 sm:mt-16 text-center">
          <div className="bg-gradient-to-r from-slate-800/50 to-slate-700/50 rounded-2xl p-6 sm:p-8 backdrop-blur-sm">
            <h3 className="text-xl sm:text-2xl font-semibold mb-6 text-white">MERN Stack Proficiency</h3>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
              {[
                ...
                <div key={tech.name} className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold mb-2 bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">
                    {tech.level}
                  </div>
                  <div className="text-sm sm:text-base text-gray-300 font-medium">{tech.name}</div>
                </div>
              ))}
            </div>
          </div>
